...
function TeamBadgeLogo({ team }) {
  ...
  return (
    <img src={LOGOS[team]} alt={team} onError={() => setErr(true)} style={{ width: 16, height: 10, objectFit: 'contain' }} className="shrink-0" />
  )
}
...
function Countdown({ raceDate }) {
  ...
  useEffect(() => {
    const calc = () => {
      ...
      setTime({ days: String(d).padStart(2, '0'), hrs: String(h).padStart(2, '0'), min: String(m).padStart(2, '0') })
    }
...
export default function Dashboard({ session }) {
  ...
  const [username, setUsername] = useState(session?.user?.email?.split('@')[0] ?? 'Piloto')

  useEffect(() => {
    const load = async () => {
      const { data: player } = await supabase.from('players').select('username, favorite_team').eq('id', session.user.id).maybeSingle()
      ...
      const { data: races } = await supabase.from('races').select('*').is('results', null).order('race_date', { ascending: true }).limit(1)
      if (races?.length) {
        setNextRace(races[0])
        const { data: pred } = await supabase.from('predictions').select('race_id').eq('race_id', races[0].id).eq('player_id', session.user.id).maybeSingle()
        if (pred) setHasPrediction(true)
      }
      const { data: completed } = await supabase.from('races').select('*').not('results', 'is', null).order('race_date', { ascending: false }).limit(1)
      if (completed?.length) {
        setLastRace(completed[0])
        const { data: sc } = await supabase.from('scores').select('total_points').eq('race_id', completed[0].id).eq('player_id', session.user.id).maybeSingle()
        if (sc) setMyLastScore(sc.total_points)
      }
      const { data: allScores } = await supabase.from('scores').select('player_id, total_points, players(username, email)')
      if (allScores) {
        const totals = {}
        allScores.forEach(s => {
  ...
  return (
    <div className="px-4">
      {/* Welcome Section (Replaces the header) */}
      <div className="flex items-center gap-3 py-6">
        <div className="size-10 rounded-full flex items-center justify-center text-white font-bold text-lg ring-2" style={{ backgroundColor: favTeam ? (TEAM_COLORS[favTeam] ?? '#e00700') : '#e00700', ringColor: favTeam ? (TEAM_COLORS[favTeam] + '33') : '#e0070033' }}>
          {initials}
        </div>
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            {favTeam && (
              <span className="text-[10px] font-bold pl-1.5 pr-2 py-0.5 rounded-full leading-none flex items-center gap-1.5" style={{ backgroundColor: (TEAM_COLORS[favTeam] ?? '#888') + '25', color: TEAM_COLORS[favTeam] ?? '#888', border: `1px solid ${(TEAM_COLORS[favTeam] ?? '#888')}40` }}>
      ...
      <div className="space-y-6 pb-4">
        {loading ? <div className="h-48 bg-card-dark rounded-xl animate-pulse" /> : nextRace ? (
          <section>
            <div className="relative overflow-hidden rounded-xl bg-card-dark border border-slate-800 shadow-xl">
              <div className="absolute top-0 right-0 p-4">
                <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border ${isOpen ? 'bg-green-500/10 text-green-500 border-green-500/20' : 'bg-slate-700/50 text-slate-400 border-slate-700'}`}>
              ...
              <div className="p-5">
                ...
                <button onClick={() => isOpen ? navigate(`/predict/${nextRace.id}`) : navigate(`/race/${nextRace.id}`) } className={`w-full font-extrabold py-4 rounded-xl transition-transform active:scale-95 flex items-center justify-center gap-2 shadow-lg ${isOpen ? 'bg-primary hover:bg-primary/90 text-white shadow-primary/20' : 'bg-slate-800 hover:bg-slate-700 text-slate-300 shadow-slate-900/20'}`}>
                  {isOpen ? (hasPrediction ? 'Actualizar predicción' : 'Hacer mi prediccion') : 'Ver carrera'}
            ...
            <div className="bg-card-dark rounded-xl border border-slate-800 overflow-hidden">
              <div className="flex border-b border-slate-800">
                {[['drivers', 'Pilotos'], ['constructors', 'Constructores']].map(([key, label]) => (
                  <button key={key} onClick={() => setStandingsTab(key)} className={`flex-1 text-xs font-bold py-2.5 transition-colors ${standingsTab === key ? 'text-primary border-b-2 border-primary' : 'text-slate-500 hover:text-slate-300'}`}>
                    {label}
                  </button>
                ))}
              </div>
              {standingsTab === 'drivers' ? (
                <div className="divide-y divide-slate-800">
                  {f1Standings.drivers.slice(0,5).map((d, i) => {
                    const teamKey = CONSTRUCTOR_COLOR_MAP[d.Constructors?.[0]?.name] ?? ''; const color = TEAM_COLORS[teamKey] ?? '#555'
                    return (
                ...
                <div className="divide-y divide-slate-800">
                  {f1Standings.constructors.slice(0,5).map((c, i) => {
                    const teamKey = CONSTRUCTOR_COLOR_MAP[c.Constructor?.name] ?? ''; const color = TEAM_COLORS[teamKey] ?? '#555'
                    return (
                      <div key={i} className="flex items-center gap-3 px-4 py-2.5">
        ...
        {lastRace && (
          <section className="space-y-3">
            ...
            <button onClick={() => navigate(`/race/${lastRace.id}`)} className="w-full bg-gradient-to-br from-slate-800 to-card-dark rounded-xl p-4 border border-slate-700/50 flex items-center justify-between hover:border-primary/30 transition-colors">
              <div className="flex flex-col gap-1 text-left">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{lastRace.country_flag ?? '🏁'} {lastRace.name}</p>
